...
async def _ocr_page(
    pdf_path: str, page_number: int, zoom: float = 1.5, lang: Optional[str] = None
) -> str:
    if not OCR_AVAILABLE:
        logger.error(
            "🔴 OCR requested but dependencies (fitz, PIL, or pytesseract) are missing."
        )
        return ""
...
def _fixed_size_split_docs(
    docs: List[SimpleDoc], chunk_size: int, chunk_overlap: int, session_id: str
) -> List[Document]:
    ...
    for doc in docs:
        ...
        for t in texts:
            out.append(
                Document(
                    page_content=t, metadata={"session_id": session_id, "source": src}
                )
            )
...
async def load_and_split_with_ocr(
    ...
    async def process_single_pdf(p):
            ...
            if loaded:
                for page_index, doc in enumerate(loaded):
                    ...
                    if OCR_AVAILABLE and len(page_text) < ocr_min_chars_threshold:
                        ocr_text = await _ocr_page(
                            p, page_index, zoom=ocr_zoom, lang=ocr_lang
                        )
        ...
        if pdf_full_text:
            ...
            return [
                SimpleDoc(
                    page_content="\n".join(pdf_full_text), metadata={"source": src_name}
                )
            ]
        return []
